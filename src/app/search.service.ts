...
import {Search, SearchTv, SearhPerson} from './search';
...
export class SearchService {
  ...
  searchTv(keyword: string): Observable<SearchTv> {
    const url = `https://api.themoviedb.org/3/search/tv?api_key=${this.apiKey}&language=en-US&query=${keyword}&page=1`;
    return this.httpClient.get<SearchTv>(url).pipe(map(response => {
      return response as SearchTv;
    }));
  }
